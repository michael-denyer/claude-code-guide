...
### Where each gate type fires (and the gap that hookify fills)

The table above lists `pre-commit`, `conftest.py`, `ruff`, `commit-msg`, and CI gates. Each of these fires at a specific moment in the development loop:

| Gate type | Fires at | Catches |
| --- | --- | --- |
| `ruff` / `pyrefly` / IDE | Edit time | Style, type errors as you code |
| `prek` (pre-commit) | `git commit` | Anything you forgot to fix before staging |
| `commit-msg` hook | `git commit` (message validation) | Commit-message rules (AI attribution, format) |
| CI (workflows) | `git push` / PR | Slow / cross-environment checks (osv-scanner, full test matrix) |
| `settings.json` `deny` | Tool-call match | Dangerous bash commands by literal pattern |

**The gap**: none of these fires *while Claude is reasoning*. Claude can compose a `git commit -m "feat: X (Co-Authored-By: Claude)"` and the rule isn't checked until the commit lands. Claude can run `pip install -q ruff` and it succeeds before any check sees the `--quiet` flag. Claude can patch an internal function with `mock.patch('myproject.foo')` and CI catches it 20 minutes later, after you've already paid the iteration cost.

**[`hookify`](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/hookify)** closes this gap. It lets you write **PreToolUse / PostToolUse / Stop** hooks as markdown-with-frontmatter files (no editing `hooks.json` by hand), so prose rules in CLAUDE.md become harness-level gates that fire **before** the tool call reaches its target.

What hookify gates that the other tools can't:

- **Inspect argument *content*** — block a `git commit` if the message contains AI attribution; block a `Bash` call if the args contain `--quiet` / `--no-verify`; block an `Edit` if the patch introduces a mock against an internal package path
- **Cross-call awareness** — "if the previous Edit touched `pyproject.toml`, the next `git commit` must also stage `uv.lock`"
- **Branch / state-aware denies** — "deny `git push --force` only on protected branches" (settings.json `deny` is unconditional)
- **PostToolUse feedback into the next turn** — auto-run `pyrefly check` after every Edit/Write of `*.py`, inject diagnostics so Claude self-corrects in the same session
- **Conversational pattern detection** — count "while we're here, let me also refactor…" attempts across a session and intervene
- **Soft warnings or auto-corrections** instead of binary block — rewrite a `pytest` invocation to add `-n 3` rather than refusing it

**Recommended starting set** (four highest-leakage prose rules from this guide that have no current gate):

1. **AI attribution blocker** on `Bash` matching `git commit` — inspect the message, block on `Co-Authored-By: Claude` / `generated by Claude`
2. **`--no-verify` guard** on any `git commit` — block unless explicitly authorised
3. **`--quiet` / `-q` / `--silent` blocker** on Bash — your CLAUDE.md mandates this everywhere with zero current enforcement
4. **`uv.lock` reminder** — PostToolUse on `Edit pyproject.toml` either auto-runs `uv lock` or warns next turn

Each is roughly ten lines of markdown in hookify's format. Don't paste-bomb every CLAUDE.md rule into hooks on day one — promote your three or four highest-leakage rules first, run for a week, then expand. Noisy hooks become background noise that Claude (and you) start ignoring.
...
See [`hookify/README.md`](https://github.com/anthropics/claude-plugins-official/blob/main/plugins/hookify/README.md) for the markdown frontmatter format and worked examples.
...
### 5. `hookify` (harness-level gates)

Where `prek` enforces rules at `git commit` and CI workflows enforce at `git push`, **`hookify` enforces them at the moment Claude attempts a tool call** — closing the gap where prose rules in CLAUDE.md leak through before any commit-time check fires. See [Where each gate type fires](#where-each-gate-type-fires-and-the-gap-that-hookify-fills) for the detailed gap analysis.
...
After install, hooks live as markdown-with-frontmatter files. Recommended starting set (the four highest-leakage prose rules with no current gate):

1. AI attribution blocker on `git commit`
2. `--no-verify` guard on `git commit`
3. `--quiet` / `-q` / `--silent` blocker on Bash
4. `uv.lock` reminder when `pyproject.toml` is edited

See [`hookify/README.md`](https://github.com/anthropics/claude-plugins-official/blob/main/plugins/hookify/README.md) for the frontmatter format and worked examples. Don't paste-bomb every CLAUDE.md rule into hooks on day one — promote three or four highest-leakage rules first, run for a week, expand as they earn their keep.
...
### 8. `code-review-graph` (large repos only)
...
### 12. CI hardening (one-time per repo)
